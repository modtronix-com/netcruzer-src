 /**
 * @brief           Makes the USB port available as a "Serial Data Port".
 * @file            nz_serUSB.c
...
//Add debugging to this file. The DEBUG_CONF_USB_BUFFERED macro sets debugging to desired level, and is configured in "Debug Configuration" section of projdefs.h file
#if !defined(DEBUG_CONF_USB_BUFFERED)
    #define DEBUG_CONF_USB_BUFFERED       DEBUG_CONF_DEFAULT   //Default Debug Level, disabled if DEBUG_LEVEL_ALLOFF defined, else DEBUG_LEVEL_ERROR
#endif
#define MY_DEBUG_LEVEL   DEBUG_CONF_USB_BUFFERED
#include "nz_debug.h"
...
//If "Power of 2" buffer used, size is power of 2 -AND- equal to or larger than CIRBUF_MIN_SIZE
    ...
    #if ( ((USB_TXBUF_SIZE & ~(USB_TXBUF_SIZE-1))!=USB_TXBUF_SIZE) || (USB_TXBUF_SIZE < CIRBUF_MIN_SIZE))
    #error "USB_TXBUF_SIZE is not power of 2, AND larger or equal to CIRBUF_MIN_SIZE!"
    #endif

    //Ensure size is power of 2 -AND- equal to or larger than CIRBUF_MIN_SIZE
    #if ( ((USB_RXBUF_SIZE & ~(USB_RXBUF_SIZE-1))!=USB_RXBUF_SIZE) || (USB_RXBUF_SIZE < CIRBUF_MIN_SIZE))
    #error "USB_RXBUF_SIZE is not power of 2, AND larger or equal to CIRBUF_MIN_SIZE!"
    #endif
#endif
...
//module without SERPORTs. If NOT defined, this modules requires nz_serDataPorts.c
    ...
    CIRBUF  cbufTxUsb;
    BYTE    cbufTxUsb[USB_TXBUF_SIZE];   //Buffer used for USB Tx

    CIRBUF  cbufRxUsb;
    BYTE    cbufRxUsb[USB_RXBUF_SIZE];   //Buffer used for USB Rx
#endif
...
// Variables and Funtion Prototypes /////////////
#if (USB_CONTROLS_SYSLED == 1)
...
} USB_CMD_AND_DBG_FLAGS;

USB_CMD_AND_DBG_FLAGS usbCmdDbgFlags;
...
void serUSBTask(void) {
        ...
        if(!HIDRxHandleBusy(USBOutHandle))
        {
            ...
            switch(PacketFromPC.Command)
            {
                ...
                case CMDUSB_DEBUG_MESSAGE:
                    ...
                    break;
                #endif
                //We received a "Device Info" request. Return Board ID and Revision
                case CMDUSB_DEVICE_INFO:
                    ...
                    usbCmdDbgFlags.flags.bDeviceInfoCmdSent = TRUE;
                    break;
                ...
                //If the application handles USB commands, add them to the USB RX Buffer
                ...
                default:
                    ...
                    //!!!! Don't use PacketFromPC.Size any more after next line, it is overwritten! It is PacketFromPC.CommandData[0]
    ...
    if(!HIDTxHandleBusy(USBInHandle))
    {
        ...
        //strings.
        if (cbufHasData(CIRBUF_TX_DEBUG) && usbCmdDbgFlags.flags.bDeviceInfoCmdSent) {
            //This method uses cbufGetRdArrSize() and cbufGetRdArr() to get contiguous blocks of data, and nzMemCpy() to copy them
            WORD sizeAvailable = cbufGetRdArrSize(CIRBUF_TX_DEBUG);     //Get number bytes available in contiguous packet to send
            ...
            PacketToPC.Command = CMDUSB_DEBUG_MESSAGE;          //Debug Message
            nzMemCpy(PacketToPC.Data, cbufGetRdArr(CIRBUF_TX_DEBUG), size);
            cbufRemoveBytes(CIRBUF_TX_DEBUG, size);

//            //This method uses cbufGetRdArrSize() and cbufGetRdArr() to get contiguous blocks of data, and nzMemCpy() to copy them
//            //Set size to maximum bytes that can be sent per HID USB message
//            PacketToPC.Command = CMDUSB_DEBUG_MESSAGE;          //Debug Message
//            size = sizeof(PacketToPC.Data);
//            PacketToPC.Size = cbufGetArray(CIRBUF_TX_DEBUG, PacketToPC.Data, size);
//            cbufEmpty(CIRBUF_TX_DEBUG);
//            //cbufRemoveBytes(CIRBUF_TX_DEBUG);

            //This method uses cbufGetByte_MACRO() for each byte it copies
//            WORD sizeAvailable = cbufGetCount(CIRBUF_TX_DEBUG);  //Bytes available to send
//            //Set size to maximum bytes that can be sent per HID USB message
//            size = sizeof(PacketToPC.Data);
//            //There are less bytes to send than bytes we ALWAYS send in standard USB message
//            if (size > sizeAvailable) {
...
//            }
    ...
    //This code if from the Webserver, and is NOT required for this module
    //Delete at a later stage!!!!!!!!!!!!!!!!!!!!!!!!!!
    BYTE command;
    BOOL done = TRUE;
    ...
    switch (smSerUSB) {
        case SM_SERUSB_IDLE:
            ...
            if (cbufHasData(CIRBUF_RX_USB)) {
                ...
                if (command == CMDUSB_COMMAND) {
                    //DEBUG_PUT_STR(DEBUG_LEVEL_INFO, "\nserUSBTask CMD Fnd");
                    done=FALSE;
                    smSerUSB = SM_SERUSB_COMMAND;
                }
                //If unknown command, remove from buffer
                else {
        ...
        case SM_SERUSB_COMMAND:
            ...
            if (nz_cmdReadyToExec() == FALSE) {
                return;     //Return, we have to execute a command
            }

            //Parse all bytes contained in Packed in USB Buffer (CIRBUF_RX_USB)
            ret = nz_cmdParseCirbuf(CIRBUF_RX_USB, sizeData, CIRBUF_TX_USB, CMDUSB_REQUEST_COMMAND);
        ...
        case SM_SERUSB_COMMAND_WAIT_REPLY:
            ...
            if (nz_cmdCommandReplyReady() == TRUE) {
...
void blinkUSBStatus(void) {
    ...
    /////////////////////////////////////////////////
    //Get blink pattern of System LED
    ...
    /////////////////////////////////////////////////
    //Blink System LED to value contained in sysLedPattern
