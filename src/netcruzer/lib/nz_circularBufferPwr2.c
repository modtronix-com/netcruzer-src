...
void cbufInitDefault(CIRBUF* pBuf, BYTE* bufArray, WORD size)
{
    ...
    pBuf->flagVal = CIRBUF_FLAGS_POWER2 | CIRBUF_TYPE_STREAMING | CIRBUF_FORMAT_NONE;
...
{
//For <b>Multi Threaded Applications</b>, when calling this function from "PUT context", code
//below is not required. Worst that can happen is a function that modifies GET can be called half
//way thru this function. This will not have any negative affect.
//    WORD putCpy, getCpy;
//
...
//
//    return ( ((putCpy+1) & (pBuf)->maxOffset) == getCpy);

    return ( (((pBuf)->put+1) & (pBuf)->maxOffset) == (pBuf)->get);
}
...
WORD cbufGetCount(CIRBUF* pBuf)
{
    ...
    return (putCpy - getCpy) & (pBuf)->maxOffset;
}
...
WORD cbufGetFree(CIRBUF* pBuf)
{
    ...
    return (getCpy - putCpy - 1) & (pBuf)->maxOffset;
}
...
BYTE cbufPeekByteAt(CIRBUF* pBuf, WORD offset) {
    return (pBuf->buf[ ((pBuf->get + offset) & pBuf->maxOffset) ]);
}
#endif
...
WORD cbufFindByte(CIRBUF* pBuf, WORD offset, BYTE value) {
    ...
    while(1) {
        for (i=0; i<sizeCtgs; i++) {
            //Byte found, return offset of byte = i
            if (*pCtgs++ == value) {
                return (firstLoop ? i : (i+rdArrSize));
            }
        }
        ...
        if (firstLoop && (cbufGetCount(pBuf) > sizeCtgs)){
            ...
            //Searched both contiguous blocks, and not found. Return not found.
            break;
        }
    };
...
void cbufRemoveByte(CIRBUF* pBuf) {
    pBuf->get = ((pBuf->get + 1) & pBuf->maxOffset);
}
#endif
...
void cbufRemoveBytes(CIRBUF* pBuf, WORD n) {
    pBuf->get = ((pBuf->get + n) & pBuf->maxOffset);
}
#endif


void cbufRemovePutByte(CIRBUF* pBuf) {
    pBuf->put = ((pBuf->put - 1) & pBuf->maxOffset);
}
...
BYTE cbufShaddowPutByte(CIRBUF* pBuf, BYTE b, WORD putShaddow) {
    ...
    //Write byte to buf
    return ((putShaddow + 1) & pBuf->maxOffset);
}
 ...
 * is '^'. When reading back the byte with cbufGetEscapedByte, the original bytes will be
...
WORD cbufPutString(CIRBUF* pBuf, const char *s) {
    ...
    // TODO!
    // This function was copied from nz_circularBufferStd. See if it can be optimized for nz_circularBufferPwr2!

    //Get string length, EXCLUDING NULL terminator. For the string "Hi" it will be 2
    p = (char*)s;
    strLen = 0;
    while (*p++ != 0) {
        strLen++;
...
WORD cbufPutAsciiEscString(CIRBUF* cbufDst, CIRBUF* cbufSrc, const char* pStrSrc, WORD flags) {
    ...
    //Check if the destination is a Packet, and this is NOT supported
    ...
    while(1) {
        ...
        if (putAvailable <= 0) {
            putAvailable = -1;  //Set error condition
            break;
        }
    ...
    //so writing to buffer will use original PUT pointer.
    #if !defined(CIRBUF_DISABLE_PACKETS) && defined(CIRBUF_PUTASCIIESC_SUPPORT_DST_PACKETS) //Packets ARE supported
    if (cbufIsPacket(cbufDst)) {
        DEBUG_PUT_STR(DEBUG_LEVEL_INFO, "\nUpdating PctSize=");
        ...
        if (cbufIsPacketLarge(cbufDst)) {
            ...
            cbufPutByteNoCheck(cbufDst, (BYTE)(bytesWritten>>8));
        }
        cbufPutByteNoCheck(cbufDst, (BYTE)bytesWritten);
    }
    #endif  //#if !defined(CIRBUF_DISABLE_PACKETS)
...
/**
 * Indicates if buffer contains a whole packet.
 *
 * For <b>Multi Threaded Applications</b>, call from "GET context".
 *
 * @param pBuf Pointer to CIRBUF structure
 *
 * @return Returns 0 if false, else size of whole Packet (size of whole packet = [Size] and [Data] part of packet).
 */
...
WORD cbufGetContiguousPacket(CIRBUF* pBuf, BYTE** ppData)
{
    ...
    //This function is not implemented yet for when CIRBUF_DISABLE_CONTIGUOUS_PACKETS is defined
...
WORD cbufPeekPacketDataSize(CIRBUF* pBuf) {
    ...
    #if (MY_DEBUG_LEVEL >= DEBUG_LEVEL_ERROR)
    if (pBuf->flagBits.bPacket == FALSE) {
        DEBUG_PUT_STR(DEBUG_LEVEL_ERROR, "\ncbufGetPacketDataSize() not packet!");
        return 0;
    }
    #endif
    ...
    //There are possibly two contiguous blocks! First till end of buffer, then from start till GET pointer
        ...
        if (sizeRequired > blockEnd) {
            ...
            if ((getCpy == 0) || (sizeRequired >= getCpy)) {
                ...
                //!!!!!!! MAYBE !!!!!!!!!!!!
                //If 2 contiguous spaces, and both are too small, write dummp packet to end of buffer
                //This will cause next butGetXxx function to remove the dummy packet, and buffer will
                //be reset to 0
