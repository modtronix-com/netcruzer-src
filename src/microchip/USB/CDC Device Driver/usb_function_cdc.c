...
void CDCTxService(void)
{
    ...
    else if(cdc_trf_state == CDC_TX_BUSY)
    {
        ...
        //putUSBUSART and putsUSBUSART set the pCDCSrc.bRam buffer to the data supplied by user. This data
        //is copied below to the CDC buffer cdc_data_tx. In the USBTxOnePacket() (defined as USBTransferOnePacket() )
        //function, this given data is scheduled to be transmitted - set bits and bytes in USB Hardware BD. Will be transmitted
        //in background by USB peripheral. Call USBHandleBusy() to determine when done.
        //MODTRONIX end
        else // _RAM
        {
            while(i)
