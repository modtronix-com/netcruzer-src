...
#if !defined(DEBUG_CONF_TCP)
    #define DEBUG_CONF_TCP        DEBUG_CONF_DEFAULT   //Default Debug Level, disabled if DEBUG_LEVEL_ALLOFF defined, else DEBUG_LEVEL_ERROR
#endif
#define MY_DEBUG_LEVEL   DEBUG_CONF_TCP
#include "nz_debug.h"
...
TCP_SOCKET TCPOpen(DWORD dwRemoteHost, BYTE vRemoteHostType, WORD wPort, BYTE vSocketPurpose)
{
    ...
    DEBUG_PUT_STR(DEBUG_LEVEL_ERROR, "\nTCP: No Skt Avlbl");    //MODTRONIX added this line
...
BOOL TCPPut(TCP_SOCKET hTCP, BYTE byte)
{
	...
	if(wFreeTXSpace == 0u) {
        DEBUG_PUT_STR(DEBUG_LEVEL_ERROR, "\nTCP: TCPPut no space");    //MODTRONIX added this line
		return FALSE;
    }
...
WORD TCPPutArray(TCP_SOCKET hTCP, BYTE* data, WORD len)
{
	...
	{
		TCPFlush(hTCP);
        DEBUG_PUT_STR(DEBUG_LEVEL_ERROR, "\nTCP: TCPPutArray no space");    //MODTRONIX added this line
		return 0;
	}
...
void TCPTick(void)
{
	...
	for(hTCP = 0; hTCP < TCP_SOCKET_COUNT; hTCP++)
	{
		...
		{
			// See if the timeout has occurred, and we need to send a new window update and pending data
		...
		{
			// See if the timeout has occurred and delayed ACK needs to be sent
		...
		if(MyTCBStub.smState == TCP_CLOSE_WAIT)
		{
			...
			if((SHORT)(MyTCBStub.OverlappedTimers.closeWaitTime - (WORD)TickGetDiv256()) <= (SHORT)0)
			{
				vFlags = FIN | ACK;
				MyTCBStub.smState = TCP_LAST_ACK;
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Forced close after wait");    //MODTRONIX added this line
			}
		}
		...
		if(!MyTCBStub.Flags.bTimerEnabled)
		{
				...
				{
					// If timeout has not occurred, do not do anything.
					if((LONG)(TickGet() - MyTCBStub.eventTime) < (LONG)0)
						continue;
		
					// If timeout has occurred and the connection appears to be dead (no 
					...
					{
                        DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Conn Closed - Timeout");    //MODTRONIX added this line
					...
					// Otherwise, if a timeout occurred, simply send a keep-alive packet
					SyncTCB();
		...
		// A timeout has occurred.  Respond to this timeout condition
		...
		switch(MyTCBStub.smState)
		{
			...
			#if defined(STACK_USE_DNS)
			case TCP_GET_DNS_MODULE:
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout GET_DNS");    //MODTRONIX added this line
				if(DNSBeginUsage())
				{
			...
			case TCP_DNS_RESOLVE:
			{
                ...
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout DNS_RESOLVE");    //MODTRONIX added this line
			...
			case TCP_GATEWAY_SEND_ARP:
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout GATEWAY_SEND_ARP");    //MODTRONIX added this line
			...
			case TCP_GATEWAY_GET_ARP:
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout GATEWAY_GET_ARP");    //MODTRONIX added this line
			...
			case TCP_SYN_SENT:
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout SYN_SENT");    //MODTRONIX added this line
			...
			case TCP_SYN_RECEIVED:
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout SYN_RCVD");    //MODTRONIX added this line
				...
				{
                    DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout SYN_RCVD Max");    //MODTRONIX added this line
			...
			case TCP_ESTABLISHED:
			case TCP_CLOSE_WAIT:
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout ESTABLSHD");    //MODTRONIX added this line
				...
				{
                    DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout ESTABLSHD Max");    //MODTRONIX added this line
			...
			case TCP_FIN_WAIT_1:
				...
				{
                    DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout FIN_WAIT Max");    //MODTRONIX added this line
			...
			case TCP_FIN_WAIT_2:
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout FIN_WAIT2");    //MODTRONIX added this line
				// Close on our own, we can't seem to communicate 
				// with the remote node anymore
				vFlags = RST | ACK;
				bCloseSocket = TRUE;
			...
			case TCP_CLOSING:
				...
				{
                    DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout CLOSING Max");    //MODTRONIX added this line
					// Close on our own, we can't seem to communicate 
					// with the remote node anymore
					vFlags = RST | ACK;
					bCloseSocket = TRUE;
...
//			
			...
			case TCP_LAST_ACK:
				...
				{
                    DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout LAST_ACK Max");    //MODTRONIX added this line
					vFlags = RST | ACK;
					bCloseSocket = TRUE;
				}
				break;
		...
		for(w = 0; w < TCP_SYN_QUEUE_MAX_ENTRIES; w++)
		{
			...
			{
                DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nTCP: Tmout SYN_Q");    //MODTRONIX added this line
				// Delete this SYN from the SYNQueue and compact the SYNQueue[] array
				TCPRAMCopy((PTR_BASE)&SYNQueue[w], TCP_PIC_RAM, (PTR_BASE)&SYNQueue[w+1], TCP_PIC_RAM, (TCP_SYN_QUEUE_MAX_ENTRIES-1u-w)*sizeof(TCP_SYN_QUEUE));
				SYNQueue[TCP_SYN_QUEUE_MAX_ENTRIES-1].wDestPort = 0u;
