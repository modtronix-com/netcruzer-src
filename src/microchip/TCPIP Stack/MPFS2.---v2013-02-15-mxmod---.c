...
#if !defined(DEBUG_CONF_MPFS2)
    #define DEBUG_CONF_MPFS2        DEBUG_CONF_DEFAULT   //Default Debug Level, disabled if DEBUG_LEVEL_ALLOFF defined, else DEBUG_LEVEL_ERROR
#endif
#define MY_DEBUG_LEVEL   DEBUG_CONF_MPFS2
#include "nz_debug.h"
 ...
 *     If MPFS2_FLAG_HASINDEX flag is set, file following this one will be index file. Index files do not
 *     have filenames.
 *
...
MPFS_HANDLE MPFSOpenID(WORD hFatID)
{
	...
	for(hMPFS = 1; hMPFS <= MAX_MPFS_HANDLES; hMPFS++)
		if(MPFSStubs[hMPFS].addr == MPFS_INVALID)
			break;
	if(hMPFS == MAX_MPFS_HANDLES) {
        DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nMPFS: No Hndl avlble - MPFSOpenID");
		return MPFS_INVALID_HANDLE;
    }
...
WORD MPFSPutArray(MPFS_HANDLE hMPFS, BYTE* cData, WORD wLen)
{
		...
		SPIFlashWriteArray(cData, wLen, TRUE);
		return wLen;
	#endif
}
#endif
...
DWORD MPFSGetTimestamp(MPFS_HANDLE hMPFS)
{
	// Make sure a valid file is open
	if(hMPFS > MAX_MPFS_HANDLES) {
        DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nMPFS: Invld Hndl - MPFSGetTimestamp");
		return 0x00000000;
    }
	if(MPFSStubs[hMPFS].addr == MPFS_INVALID)
		return 0x00000000;
...
DWORD MPFSGetMicrotime(MPFS_HANDLE hMPFS)
{
	// Make sure a valid file is open
	if(hMPFS > MAX_MPFS_HANDLES) {
        DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nMPFS: Invld Hndl - MPFSGetMicrotime");
		return 0x00000000;
    }
	if(MPFSStubs[hMPFS].addr == MPFS_INVALID)
		return 0x00000000;
...
MPFS_PTR MPFSGetStartAddr(MPFS_HANDLE hMPFS)
{
	// Make sure a valid file is open
	if(hMPFS > MAX_MPFS_HANDLES) {
        DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nMPFS: Invld Hndl - MPFSGetStartAddr");
		return 0;
    }
	if(MPFSStubs[hMPFS].addr == MPFS_INVALID)
		return MPFS_INVALID;
...
MPFS_PTR MPFSGetEndAddr(MPFS_HANDLE hMPFS)
{
	// Make sure a valid file is open
	if(hMPFS > MAX_MPFS_HANDLES) {
        DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nMPFS: Invld Hndl - MPFSGetEndAddr");
		return MPFS_INVALID;
    }
	if(MPFSStubs[hMPFS].addr == MPFS_INVALID)
		return MPFS_INVALID;
...
BOOL MPFSGetFilename(MPFS_HANDLE hMPFS, BYTE* cName, WORD wLen)
{
	...
	// Make sure a valid file is open
	if(hMPFS > MAX_MPFS_HANDLES) {
        DEBUG_PUT_STR(DEBUG_LEVEL_WARNING, "\nMPFS: Invld Hndl - MPFSGetFilename");
		return FALSE;
    }
	if(MPFSStubs[hMPFS].addr == MPFS_INVALID)
		return FALSE;
