...
/****************************************************************************
  ...
  ***************************************************************************/
    ...
    //MODTRONIX added. Must correspond to HTTPResponseHeaders in HTTP2.c !!!
...
/****************************************************************************
  ...
  ***************************************************************************/
    ...
    // Content-Type constants used to check type of data received via HTTP POST. MUST correspond to HTTPPostContentTypeHeaders[] in HTTP2.c!!
	typedef enum
	{
		POST_CONTENT_TYPE_MUL_FORM_DATA = 0,                // multipart/form-Data
        POST_CONTENT_TYPE_APP_X_WWW_FORM_URLENCODED,        // application/x-www-form-urlencoded
        POST_CONTENT_TYPE_UNKNOWN                           // unknown
	} POST_CONTENT_TYPE;
    #define POST_CONTENT_TYPE_SIZE (POST_CONTENT_TYPE_UNKNOWN)
	...
	typedef struct
	{
		...
		DWORD nextCallback;					// Byte index of the next callback
                                            // - Is used when sending files to client. Is used from SM_HTTP_PROCESS_REQUEST to SM_HTTP_SERVE_BODY (used in HTTPSendFile()).
                                            //   Before SM_HTTP_PROCESS_REQUEST state, it is used to pass data to POST processing functions
                                            // - Is used to pass information to POST processing functions. From SM_HTTP_PARSE_REQUEST to SM_HTTP_PARSE_HEADERS (use in HTTPHeaderParseLookup())
        ...
        BYTE *ptrCookies;                   // Points to start of cookies in data
		...
		BYTE smPost;						// POST state machine variable. Can have two meanings! Values below 0x80 are actual POST states.
                                            // Values 0x80 and above are used to store 'Content-Type' of POST message, and are POST_CONTENT_TYPE_xxx
                                            // constants. In POST handler function, first check if smPost >= 0x80 to determine if this is first time handler is called.
		#endif
	} HTTP_CONN;
