...
//MODTRONIX added next 6 lines
HTTP_IO_RESULT cmdParseHTTPPost(void);              //Prototype of function defined in cmd.c
#if !defined(DEBUG_CONF_CUSTOM_HTTP_APP)
    #define DEBUG_CONF_CUSTOM_HTTP_APP        DEBUG_CONF_DEFAULT   //Default Debug Level, disabled if DEBUG_LEVEL_ALLOFF defined, else DEBUG_LEVEL_ERROR
#endif
#define MY_DEBUG_LEVEL   DEBUG_CONF_CUSTOM_HTTP_APP
#include "nz_debug.h"
...
HTTP_IO_RESULT HTTPExecuteGet(void)
{
    ...
    //MODTRONIX added next 8 lines
    //At this stage the curHTTP.data buffer will have all GET name=value args, AND cookies.
    //The GET name/value pairs were added in the SM_HTTP_PARSE_REQUEST state of HTTPProcess().
    //The Cookie name/value pairs were added in the SM_HTTP_PARSE_HEADERS state of HTTPProcess().
    //In SM_HTTP_PARSE_HEADERS state, the HTTPHeaderParseLookup() function calls HTTPHeaderParseCookie()
    //that added all cookies found in header.
    //Each name and value string is NULL terminated. The end is indicated by an additional NULL.
    //For example: "name1[NULL]value1[NULL]name2[NULL]value2[NULL][NULL]"
    //
    //!!!!! IMPORTANT !!!!!
    //In addition, we can add Cookie name-value pairs to curHTTP.data, that will be sent to the client
    //as new Cookies. DON'T CONFUSE this with cookies received from client that are included with GET
    //name-value pairs, as described above!
    //At this stage curHTTP.hasArgs will be 0 (cleared in SM_HTTP_AUTHENTICATE state). If nzcks.htm
    //was requested, all parameters MUST be cookies! They will already be in curHTTP.data (up to
    //curHTTP.ptrCookies) as name/value pairs. From curHTTP.ptrCookies are the received Cookies.
    //Count how many name/value params we have, and set curHTTP.hasArgs to this value.
    //This is processed in SM_HTTP_SERVE_HEADERS state of HTTPProcess(), and sent to browser as new
    //cookies. These new cookies added will from now on always be included in every request the browser
    //sends us.
    //
    //In future, consider only allowing cookies to be added via POST. If this is done, we will NOT add
    //any cookies here (not increment curHTTP.hasArgs). But, will have cookie processing in HTTPExecutePost().
    //In this case, we will search for any cookies received via POST, add them to curHTTP.data, and increment
    //curHTTP.hasArgs.
    ...
    //Check for special file types, start with "nz..."
    //if ((filename[0]=='n') && (filename[1]=='z') {
    if(memcmppgm2ram(filename, "nzcks.htm", 9) == 0)
    {
        //Mark that this is "nzcks.htm". Variable curHTTP.hasArgs would have been 0 at this stage.
        //We indicate how many cookies are contained in name/value pairs. Remember to decrement curHTTP.hasArgs
        //by 1 before existing!
        curHTTP.hasArgs = 1;
    }
    //}

    char* ptrValue;
    //curHTTP.data contains name-value pairs. Get them all, and send them to cmdExecNameValueCmd()
    //for command processing.
    ptr = curHTTP.data;
    // Search through the array while bytes remain
    while(*ptr != '\0')
    {
        ptrValue = (char*)(ptr + strlen((char*)ptr) + 1);
        DEBUG_PUT_STR(DEBUG_LEVEL_INFO, "\ncmdGet name=");
        DEBUG_PUT_STR(DEBUG_LEVEL_INFO, (const char*)ptr);

        DEBUG_PUT_STR(DEBUG_LEVEL_INFO, ", value=");
        DEBUG_PUT_STR(DEBUG_LEVEL_INFO, (const char*)(ptr + strlen((char*)ptr) + 1));

        //This is a Cookie request.
        if (curHTTP.hasArgs!=0) {
            //Ensure we only send GET name-value pairs (contained below curHTTP.ptrCookies)
            //From above curHTTP.ptrCookies are the cookies we received from the client.
            // - If this is NOT a received Cookie (ptr < curHTTP.ptrCookies)
            // - If there were no received Cookies (curHTTP.ptrCookies==0)
            if ((ptr<curHTTP.ptrCookies) || (curHTTP.ptrCookies==0)) {
                DEBUG_PUT_STR(DEBUG_LEVEL_INFO, " ck");
                curHTTP.hasArgs++;  // For each cookie, increment curHTTP.hasArgs.
            }
        }
        else {
            ...
            cmdExecNameValueCmd((char*)ptr, ptrValue, (char*)filename, 0);
        }

        // Skip past two strings (name-value pair) (NUL bytes)
        ptr = ((BYTE*)ptrValue) + (strlen((char*)ptrValue) + 1);
    }

    //Decment by 1. Was set to 1 as marker.
    if (curHTTP.hasArgs!=0) {
        curHTTP.hasArgs--;
    }
...
#if defined(STACK_USE_HTTP_APP_RECONFIG)
static HTTP_IO_RESULT HTTPPostConfig(void) {
    //Is used to store CFG_BLOCK_NET, minus the 'reserve' array at end of structure
    BYTE newNetConfig[CFG_BLOCK_SIZE(net)];
    ...
    // in memory and using it instead of newNetConfig.  Lastly, you could
    ...
    xeeReadArray(XEEMEM_CFG_STRUCT_START_ADR + offsetof(CFG_STRUCT, net),
            newNetConfig,
            sizeof (newNetConfig));
    ...
    // submitted.
    ((CFG_BLOCK_NET *)newNetConfig)->Flags.bIsDHCPEnabled = 0;
	...
	while(curHTTP.byteCount)
    {
        ...
        if (!strcmppgm2ram((char*) curHTTP.data, (ROM char*) "ip")) {// Read new static IP Address
            if (!StringToIPAddress(curHTTP.data + 6, &((CFG_BLOCK_NET *)newNetConfig)->MyIPAddr))
                goto ConfigFailure;

            ((CFG_BLOCK_NET *)newNetConfig)->DefaultIPAddr.Val = ((CFG_BLOCK_NET *)newNetConfig)->MyIPAddr.Val;
        } else if (!strcmppgm2ram((char*) curHTTP.data, (ROM char*) "gw")) {// Read new gateway address
            if (!StringToIPAddress(curHTTP.data + 6, &((CFG_BLOCK_NET *)newNetConfig)->MyGateway))
                goto ConfigFailure;
        } else if (!strcmppgm2ram((char*) curHTTP.data, (ROM char*) "sub")) {// Read new static subnet
            if (!StringToIPAddress(curHTTP.data + 6, &((CFG_BLOCK_NET *)newNetConfig)->MyMask))
                goto ConfigFailure;

            ((CFG_BLOCK_NET *)newNetConfig)->DefaultMask.Val = ((CFG_BLOCK_NET *)newNetConfig)->MyMask.Val;
        } else if (!strcmppgm2ram((char*) curHTTP.data, (ROM char*) "dns1")) {// Read new primary DNS server
            if (!StringToIPAddress(curHTTP.data + 6, &((CFG_BLOCK_NET *)newNetConfig)->PrimaryDNSServer))
                goto ConfigFailure;
        } else if (!strcmppgm2ram((char*) curHTTP.data, (ROM char*) "dns2")) {// Read new secondary DNS server
            if (!StringToIPAddress(curHTTP.data + 6, &((CFG_BLOCK_NET *)newNetConfig)->SecondaryDNSServer))
                goto ConfigFailure;
...
static HTTP_IO_RESULT HTTPPostSNMPCommunity(void)
{
	...
	switch(curHTTP.smPost)
    {
    	case SM_CFG_SNMP_READ_NAME:
            ...
            {
                //MODTRONIX Replaced this line with line below it. Have not tested it jet!
                //cfgSaveAppConfig(&AppConfig);
                cfgSaveArray(offsetof(CFG_STRUCT, snmp),
                    (BYTE *)&AppConfig.readCommunity[0][0],     //Get address in AppConfig of start of CFG_BLOCK_SNMP structure
                    (sizeof(CFG_BLOCK_SNMP) -  sizeof(((CFG_BLOCK_SNMP*)0)->reserve)));
...
/*****************************************************************************
  ...
  Summary:
	Processes the file upload form on uploadfile.htm
...
void HTTPPrint_led(WORD num) {
    // Determine value of LEDs 7 and 8 on the DB66DEV1 development board. LEDs 7 and 8
    //are on port 28 and 29 (old port names T6 and T7).
	switch(num)
    {
    	case 7:
        	num = LAT_28;
