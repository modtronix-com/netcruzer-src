...
#define MSG_TYPE_TX_LOG         1
#define MSG_TYPE_TAMPER_ALARM   2
#define MSG_TYPE_CONFIG         5
#define MSG_TYPE_RETRANSMIT     6
...
int main(void) {
    ...
    iPPSOutput(PPS_OUT_06, OUT_FN_PPS_U1TX);        //6 (old port name X6) is TX
    iPPSOutput(PPS_OUT_08, OUT_FN_PPS_U1RTS);       //8 (old port name X8) is RTS
    iPPSInput(IN_FN_PPS_U1RX, PPS_IN_07);           //7 (old port name X7) is RX
    iPPSInput(IN_FN_PPS_U1CTS, PPS_IN_01);          //1 (old port name X1) is CTS
    CloseUART1();
